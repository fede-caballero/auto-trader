...
def run_paper_trade():
    ...
    logger.info(f"Signal: {signal} | Confidence: {confidence:.2f}% | BTC: ${price:,.2f}")
    ...
    if state["position"]:
        ...
        if should_close or signal_reversal:
            ...
            logger.info(log_msg)
            send_webhook_notification("CLOSE_POSITION", "BTCUSDT", price, log_msg)
            state["position"] = None
    ...
    if state["position"] is None and confidence >= CONFIDENCE_THRESHOLD:
        ...
        logger.info(log_msg)
        send_webhook_notification("OPEN_POSITION", "BTCUSDT", price, log_msg)
